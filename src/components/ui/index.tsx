    ...
    <div className="relative bg-gray-50 rounded-lg p-2.5">
      ...
      <input
        ...
        className={`w-full text-sm font-medium text-gray-900 bg-white border border-gray-200
          rounded-md px-2 py-1.5 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 ${className ?? ''}`}
      />
    </div>
  )
}
...
// ── Section header ────────────────────────────────────────────────────────
export function SectionHeader({ title, tooltip, children }: { title: string; tooltip?: string; children?: React.ReactNode }) {
  return (
